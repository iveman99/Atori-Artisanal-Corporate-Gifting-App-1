...
class Mugs : AppCompatActivity() {
    ...
    private fun fetchNewProductsFromFirestore() {
        ...
        db.collection("Product")
            .whereEqualTo("productCategory", "mugs and flasks")
            .get()
            .addOnSuccessListener { documents ->
                ...
                Log.d("Firestore", "Loaded ${newProduct.size} new products")
                showLayout()
            }
            .addOnFailureListener { e ->
                Log.e("Firestore", "Error loading new products", e)
                Toast.makeText(this, "Failed to load New products", Toast.LENGTH_SHORT).show()
                showLayout()
            }
    }
    ...
    private fun fetchSaleProductsFromFirestore() {
        val db = FirebaseFirestore.getInstance()
        db.collection("Product")
            .whereEqualTo("productCategory", "flasks")
            .get()
            .addOnSuccessListener { documents ->
                saleProduct.clear()
                for (document in documents) {
                ...
                Log.d("Firestore", "Loaded ${saleProduct.size} sale products")
            }
            .addOnFailureListener { e ->
                Log.e("Firestore", "Error loading sale products", e)
                Toast.makeText(this, "Failed to load Sale products", Toast.LENGTH_SHORT).show()
            }
    }
}
